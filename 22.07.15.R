...
#df_out <- read.csv(output_file, header=T)
class(df)
...
# 결과 저장 및 파일 추출
RESULT <- data.frame(df$주문번호, df$주문일시, df$주문주기, df$주문자ID, df$총.결제금액, df$`주문횟수`)
names(RESULT) <- c("첫주문번호","첫주문일자","평균주문주기","ID","총주문금액","주문횟수")
RESULT <- relocate(RESULT,c(ID,첫주문일자,첫주문번호,평균주문주기,주문횟수,총주문금액))
