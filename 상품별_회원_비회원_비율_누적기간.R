# 4-6월 간 회원 데이터와 같은 cafe24의 '주문조회>전체주문조회>엑셀다운로드>전체데이터'를 통해 다운받은 파일을 입력하면,
#해당 기간동안 주문상품별로 '주문 수'와 '주문수량'의 회원/비회원 '인원 수'와 그 비율을 결과 파일로 출력해준다.

# 불러올 데이터프레임이 포함된 폴더 위치 line5 '' 사이에 입력
setwd('/Users/jungwooseop/모시공/행사/2022 하계 인턴십/문서/엑셀/상품별_회원_비회원_비율/20.10.21_22.06')
dir_name <- getwd()

# 불러올 데이터 파일명은 line5에 입력한 폴더안에 존재하는 파일이어야 함
# 새로 입력할 파일을 line11의 '' 사이에 입력
# 기존에 존재하던 결과파일명을 line12의 '' 사이에 입력
...
# 기존 결과값 전처리 - 기존 누적 데이터 연산이 가능한 실수화
df_product_result_origin$총_주문_수 <- as.numeric(gsub("회","",df_product_result_origin$총_주문_수))
df_product_result_origin$회원_주문_수 <- as.numeric(gsub("회","",df_product_result_origin$회원_주문_수))
df_product_result_origin$비회원_주문_수 <- as.numeric(gsub("회","",df_product_result_origin$비회원_주문_수))
df_product_result_origin$총_주문수량 <- as.numeric(gsub("개","",df_product_result_origin$총_주문수량))
df_product_result_origin$회원_주문수량 <- as.numeric(gsub("개","",df_product_result_origin$회원_주문수량))
df_product_result_origin$비회원_주문수량 <- as.numeric(gsub("개","",df_product_result_origin$비회원_주문수량))
...
# integer(0)값을 판단해 bool값 return하는 함수 선언
is.integer0 <- function(x)
{
  is.integer(x) && length(x) == 0L
}

# grep() 함수 실행을 위해 주문상품명 전처리
temp_sold_num <- df_sold_num1$주문상품
temp_origin_num <- df_origin_num$주문상품
df_sold_num1$주문상품<-gsub("[[:punct:]]", "", df_sold_num1$주문상품)
df_origin_num$주문상품<-gsub("[[:punct:]]", "", df_origin_num$주문상품)

# 주문상품이 기존 데이터에 존재하는 상품이면 데이터를 병합하는 for문
for(i in 1:length(df_sold_num1$주문상품)){
  temp<-grep(paste("^",df_sold_num1$주문상품[i],"$", sep=""), df_origin_num$주문상품)
  # 상품이 누적데이터에 이미 존재 시 주문 수 통합
  if(!is.integer0(temp)){
    df_sold_num1$총_주문_수[i]    <-df_sold_num1$총_주문_수[i]    +df_origin_num$총_주문_수[temp]
    df_sold_num1$회원_주문_수[i]  <-df_sold_num1$회원_주문_수[i]  +df_origin_num$회원_주문_수[temp]
    df_sold_num1$비회원_주문_수[i]<-df_sold_num1$비회원_주문_수[i]+df_origin_num$비회원_주문_수[temp]
    df_origin_num   <- df_origin_num[-temp,]
    temp_origin_num <- temp_origin_num[-temp]
  }
}
df_sold_num1$주문상품 <- unique(temp_sold_num)
df_origin_num$주문상품 <- unique(temp_origin_num)

# 중복 되지않는 누적데이터에 존재하는 상품정보 병합
df_sold_num1<-rbind(df_sold_num1,df_origin_num)

# 총 주문수 기준 내림차순으로 데이터프레임 재정렬
...
# grep 함수 실행을 위해 주문상품명 전처리
temp_quantity <- df_quantity1$주문상품
temp_origin_quantity <- df_origin_quantity$주문상품
df_quantity1$주문상품<-gsub("[[:punct:]]", "", df_quantity1$주문상품)
df_origin_quantity$주문상품<-gsub("[[:punct:]]", "", df_origin_quantity$주문상품)

# 누적 데이터에 존재하는 주문상품의 경우 데이터 병합
for(i in 1:length(df_quantity1$주문상품)){
  temp<-grep(paste("^",df_quantity1$주문상품[i],"$", sep=""), df_origin_quantity$주문상품)
  # 추가하는 데이터의 상품이 누적데이터에 존재 시 주문수량 통합
  if(length(temp)!=0){
    df_quantity1$총_주문수량[i]    <-df_quantity1$총_주문수량[i]    +df_origin_quantity$총_주문수량[temp]
    df_quantity1$회원_주문수량[i]  <-df_quantity1$회원_주문수량[i]  +df_origin_quantity$회원_주문수량[temp]
    df_quantity1$비회원_주문수량[i]<-df_quantity1$비회원_주문수량[i]+df_origin_quantity$비회원_주문수량[temp]
    df_origin_quantity<-df_origin_quantity[-temp,]
    temp_origin_quantity <- temp_origin_quantity[-temp]
  }
}
df_quantity1$주문상품 <- unique(temp_quantity)
df_origin_quantity$주문상품 <- unique(temp_origin_quantity)

# 중복 되지않는 누적데이터에 존재하는 상품정보 병합
df_quantity1<-rbind(df_quantity1,df_origin_quantity)

# 총 주문수량 기준 내림차순으로 데이터프레임 재정렬
df_quantity1 <- df_quantity1[order(df_quantity1$총_주문수량,decreasing=TRUE),]

# 총 주문수량 대비 회원 주문수량 비중
df_quantity1 <- mutate(df_quantity1,
...
# 단위 기입이 필요한 열에 단위 추가
df_product_result$총_주문_수 <- paste(as.character(df_product_result$총_주문_수),"회", sep = "")
df_product_result$회원_주문_수 <- paste(as.character(df_product_result$회원_주문_수),"회", sep = "")
df_product_result$비회원_주문_수 <- paste(as.character(df_product_result$비회원_주문_수),"회", sep = "")
df_product_result$총_주문수량 <- paste(as.character(df_product_result$총_주문수량),"개", sep = "")
df_product_result$회원_주문수량 <- paste(as.character(df_product_result$회원_주문수량),"개", sep = "")
df_product_result$비회원_주문수량 <- paste(as.character(df_product_result$비회원_주문수량),"개", sep = "")
